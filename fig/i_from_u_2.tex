...
\pgfplotstableset{
	...
	\begin{tikzpicture}
	  ...
	  %          {$\pgfmathprintnumber[frac, frac denom=32,frac whole=false]{\pgfmathresult}\pi$}
		...
		% \addlegendentry{$U_\text{нак}=5.2$ В, $I_\text{нак}=1.48$ А}
		\addplot +[mark=*, line width=1.2pt, mark size=1.1pt,  smooth,solid,blue] plot [error bars/.cd, y dir = both, y fixed=0] table [ x=u, y=i,] {../data/vax2.tsv};
